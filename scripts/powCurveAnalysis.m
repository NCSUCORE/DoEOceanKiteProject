...
fpath = fullfile(fileparts(which('OCTProject.prj')),'Results','Manta 2.0','Rotor','400 m Tether AVL\');
for ii = 1:numel(flwSpd)
    filename = sprintf(strcat('Turb1.1_V-%.3f.mat'),flwSpd(ii));
    load([fpath filename])
    lap = max(tsc.lapNumS.Data)-1;
    [Idx1,Idx2] = tsc.getLapIdxs(lap);
    ran = Idx1:Idx2-1;
    avgPowa(ii) = mean(tsc.turbPow.Data(1,1,ran))+mean(tsc.turbPow.Data(1,2,ran));
end
%%
