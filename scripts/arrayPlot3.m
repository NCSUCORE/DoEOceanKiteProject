function arrayPlot3(arr, varargin)
%Uses Plot 3 to plot an array with at least 1 dimention with length 3
%Will not plot if array has more than 2 non singular dimentions
%Inputs:
%   arr - the array to be plotted
%   varargin - will be passed directly into the varargin of plot3
    if any(ismember(size(arr),3))
        ...
        if isempty(varargin)
            ...
            plot3(arr(1,:),arr(2,:),arr(3,:),varargin{:});
        end
    else
