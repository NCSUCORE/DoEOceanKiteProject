        ...
        for qq = 1:2
            for kk = 1:2
                ...
                if exist([fpath filename])==2
                    ...
                    rotThr = squeeze(pagemtimes(rotMat,fThr));
                    rotThr(1,:) = 0;
                    ...
                    diffDa = diff(dragAng)/dt;
                    cdThr = thrDragMag'./dynP;
                    ...
                    xTemp = [pos(3,:)'/100 pos2/100 squeeze(vRat) velAng cos(velAng) sin(velAng) squeeze(vApp)' cos(velAng).^2 sin(velAng).^2 sin(eul') cos(eul') squeeze(vRat) ];% squeeze(lift)' posMag'];% tenMag thrTen(3,:)'./thrTen(1,:)'];
                   if rand>0.25
                        xTrain = [xTrain;xTemp(1:end-1,:)];
                        drag = [drag; cdThr(1:end-1,:) diffDa];
                    else
                        xValidate = [xValidate;xTemp(1:end-1,:)];
                        dragValidate = [dragValidate;cdThr(1:end-1,:) diffDa];
                    end
                end
            end
        end
...
polyorderN = 2;
usesine = 0;
ThetaTotD = poolData(xTrain,n,polyorderN,usesine);
...
lambda = .000005;      % lambda is our sparsification knob.
XiTotA = sparsifyDynamics(ThetaTotA,drag(:,2),lambda,1);



poolDataLIST({'z','path_y','path_z','vAug'},XiTotD,n,1,polyorderN,usesine);
poolDataLIST({'z','y','zp','vAug','gam','cGam','sGam','u','v','w'},XiTotA,m,1,polyorder,usesine);
