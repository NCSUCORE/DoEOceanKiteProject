...
s_R2i_o = X_sol(15:(14 + 3*N),:);
s_O_V2i_o = X_sol((15 + 3*N):(14 + 6*N),:);
...
for i = 1:n_steps
    op = multinode_F_and_T_external(sim_param,s_flow(:,i),s_unstretched_l(:,i),s_elevon_def(:,i),...
        s_Rcm_o(:,i),s_R2i_o(:,i),...
        s_O_Vcm_o(:,i),s_O_V2i_o(:,i),s_OwB(:,i),s_euler_ang(:,i),s_platform_ang(1,i));
...
p_aero2i = plot(time./sqrt(k_scale),s_aero_2i(1,:),'LineWidth',line_wd,'color',red);
hold on
...
p_t21 = plot(time./sqrt(k_scale),s_Ts2i(i,:),'LineWidth',line_wd,'color',red);
hold on
