function [M,F] = staticMoment(Sys,Env,wing,hStab,vStab,fuse,Ang,i)
%%  Rotation Matrices 
...
pitch = Ang.pitch(i);
yaw = Ang.yaw;
vApp = Ry(pitch)*Env.vFlow-Sys.vKite;                   %   m/s - Apparent flow velocity
...
F.gravB = Ry(pitch)*F.gravG;                            %   N - Gravitational force (body)
F.buoyG = -F.gravG*Sys.B;                               %   N - Buoyancy force
F.buoyB = Ry(pitch)*F.buoyG;                            %   N - Buoyancy force (body)
%   Lift Forces
...
%   Tether Force
F.thrB = [0;0;0];                                           %   N - Tether force in the body frame
%%  Moment Calculations
...
M.tot = M.buoyB+M.W+M.H+M.V;                           %   Nm - Total moment
end
