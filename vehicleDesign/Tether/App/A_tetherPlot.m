function [support_Force, strain_Percent, power_Loss_Percent, outer_Diameter, failure_Strain_Percent,maxStrain_Percent_Over_Failure,maxStrain_Percent_Over_Failure_Componets] = A_tetherPlot(app,varMTinputs)
...
D_structure    = app.InputBoxs.Structure.Value;
...
D_wire = app.InputBoxs.Wire.Value;
fail_wireStrain =  varMTinputs.Wire.strainFailurePercent;
numWires = app.InputBoxs.numWires.Value;%varMTinputs.Wire.number;
rho_electrical = varMTinputs.Wire.rho_electrical;
helix_Angle_wire = app.InputBoxs.helixAngleWire.Value;
...
D_fiberOptic = app.InputBoxs.fiberOptic.Value;
fail_fiberOpticStrain =  varMTinputs.fiberOptic.strainFailurePercent;
numFiberOptic = app.InputBoxs.numFiberOptic.Value;
helix_Angle_fiberOptic = app.InputBoxs.helixAngleFiber.Value;
...
power = app.InputBoxs.genPower.Value;
voltage = app.InputBoxs.genVoltage.Value;
current = power/voltage;

%% Tether Properties
tension = varMTinputs.tether.tension;
l = varMTinputs.tether.length;
l_wire = l*csc(helix_Angle_wire*(pi/180));
...
k_wire_Eff = Bool_wire*((E_wire*A_wire*sind(helix_Angle_wire))/l_wire);
k_insulation_Eff = Bool_insulation*(E_insulation*A_insulation*sind(helix_Angle_wire))/l_wire;

k_fiberOptic_Eff = Bool_fiberOptic*(E_fiberOptic*A_fiberOptic*sind(helix_Angle_wire))/l_fiberOptic;
k_fiberOpticSheath_Eff = Bool_fiberOpticSheath*(E_fiberOpticSheath*A_fiberOpticSheath*sind(helix_Angle_wire))/l_fiberOptic;
...
wireStrain = Strain_total*sind(helix_Angle_wire)/(1+poisson_ratio_wire*cosd(helix_Angle_wire)^2);
wireStrainPercent = (wireStrain/l_wire)*100;
Wire_StrainDiff = wireStrainPercent - fail_wireStrain;

insulationStrain = Strain_total*sind(helix_Angle_wire)/(1+poisson_ratio_wire*cosd(helix_Angle_wire)^2);
...
for ii = 1:length(Radiuses)
    app.Plots.DrawPlot.Plot = appviscircles(app.PlotAxes.DrawPlot,Centers(ii,:),Radiuses(ii),'Color','k');
end
