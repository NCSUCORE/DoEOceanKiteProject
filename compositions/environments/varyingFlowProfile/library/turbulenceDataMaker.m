 %creator of a mat file that contains turbulence in u,v,w
env = ENV.variedFlow;

    %sets your depth array to match the oct data
                    timeVec = 0:1:3600-1;
                    env.depth.setValue((4*61+6.31),'m')
                    env.depthArray.setValue([6.31:4:4*61+6.31],'m');
                    load('usefulFlowData.mat')
                    startTime = 4000;
                    tenMinTimeInterval = ceil(3600/600);
