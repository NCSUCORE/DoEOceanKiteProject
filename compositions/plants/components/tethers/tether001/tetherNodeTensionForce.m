function [nodeForceVecs,linkForces,Scope]  = tetherNodeTensionForce(ReeledOutLength, nodePos, nodeVel, ActiveLengths, ...
...
unstretchedLengthPerscribed = ReeledOutLength; %Total Reeled out length
totalSpringConst = youngsMod*(pi/4)*(thrDiam^2)/unstretchedLengthPerscribed;
...
% 
% W = youngsMod*(pi/4)*(thrDiam^2);
% Both
% LA_Preserve = ((W.*(mass.*linkLengthDeriv.^2*dampingRatio.^2 + linkLength.*W + linkForceVecsPrev.*linkLength)).^(1/2) + linkLengthDeriv.*W.^(1/2).*dampingRatio.*mass.^(1/2)).^2./(linkForceVecsPrev + W).^2;

%First
%LA_Preserve = ((W.*(mass.*linkLengthDeriv.^2*dampingRatio.^2 + linkLength.*W + nodeForceVecsPrev.*linkLength)).^(1/2)).^2./(nodeForceVecsPrev + W).^2;

%Second
%LA_Preserve = (linkLengthDeriv.*W.^(1/2).*dampingRatio.*mass.^(1/2)).^2./(nodeForceVecsPrev + W).^2;


% 
% ActiveLengths = ActiveLengths+LA_Preserve;
% %WHOLE THING AGAIN
% 
% No = numNodes;          %Total Nodes
% Na = NumberNodesActive; %Nodes that arent reeled-in
...
%     
%     
%     nodeForceVecsPrev(:,1) = nodeForceVecsPrev(:,No-Na+1);
%     nodeForceVecsPrev(:,No-Na+1) = 0;
% end
