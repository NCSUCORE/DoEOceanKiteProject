classdef turb < handle
    ...
    methods
        function obj = turb
            ...
            obj.etaLookup            = SIM.parameter('Value',0,'Unit','','Description','Eta Lookup');
            obj.thrLookup            = SIM.parameter('Value',0,'Unit','m','Description','Effective Tether Length Lookup');
            obj.velWLookup           = SIM.parameter('Value',0,'Unit','m/s','Description','Effective Wind Velocity Lookup');
            obj.tsrLookup            = SIM.parameter('Value',0,'Unit','','Description','Tip Speed Ratio Lookup');
            obj.AoALookup            = SIM.parameter('Value',0,'Unit','','Description','AoA Lookup');
        end
        ...
        function val = get.peakTorqueCoef(obj)
            val = SIM.parameter('Value',max(obj.torqueCoefLookup.Value),'Unit','','Description','Maximum torque coefficient');
        end
        ...
        function val = get.tauCoefLookup(obj)
            [~,ind] = max(obj.torqueCoefLookup.Value);
            val = SIM.parameter('Value',...
                obj.torqueCoefLookup.Value(end:-1:ind),...
                'Unit','','Description','Torque Coef Lookup Table Data');
        end

        function val = get.cTauOpt(obj)
            [~,ind] = max(obj.CpLookup.Value./obj.CtLookup.Value);
            val = SIM.parameter('Value',...
                obj.torqueCoefLookup.Value(ind),...
                'Unit','','Description','Torque Coef Lookup Table Data');
            end
