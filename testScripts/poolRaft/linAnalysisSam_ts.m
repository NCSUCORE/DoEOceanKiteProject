    ...
    for i = 1:length(inc)
        ...
        for j = 1:length(towArray)
            ...
            for k = 1:numel(rCM)
                tic
                Simulink.sdi.clear
                h = 25*pi/180;  w = 90*pi/180;                             %   rad - Path width/height
...
%                 loadComponent('realisticSensors');                             %   Sensors
                ...
                if q ~= 3
                    ...
                    fltCtrl.rollPhase.setValue(pi-pi/2,'rad');
                ...
                op = findop('OCTModel',tsc.CDfuse.Time(end-100));
                linsys = linearize('OCTModel','OCTModel/plant',op);
...
vhcl.animateSim(tsc,0.2,'PathFunc',fltCtrl.fcnName.Value)
