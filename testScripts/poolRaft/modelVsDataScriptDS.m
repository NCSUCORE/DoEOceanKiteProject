...
for i = 1:2
    figure
    plot(tscSim{i}.rollDeg)
    hold on
%     if i ~= 1
%         plot(runData{i}.kiteRoll*180/pi*-1)
%     else
        plot(runData{i}.kiteRoll)
%     end
    legend('Sim','Exp')
    ylabel 'Roll [deg]'
...
%     end
    ...
    figure
    plot(tscSim{i}.azimuthAngle*180/pi)
    hold on
    plot(runData{i}.kite_azi)
    legend('Sim','Exp')
    ylabel 'Azimuth Angle [deg]'
    
    figure
    plot(tscSim{i}.velCMvec.mag/towArray(i))
    hold on
    [~,velAug{i}] = estExpVelMag(runData{i},1);
    plot(runData{i}.kite_azi.Time(1:end-1),velAug{i})
    xlim([1 inf])
    xlabel('Time [s]')
