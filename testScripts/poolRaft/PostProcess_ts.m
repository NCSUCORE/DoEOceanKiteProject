...
inc =3;
elevArray = 40*pi/180%[40 15]*pi/180;
towArray = [0.78];
rCM = 1
thrLength = 2.63;
flwSpd = -1e-9;
    ...
    for i = 1:length(inc)
        ...
        for j = 1:length(towArray)
            ...
            for k = 1
                tic
                Simulink.sdi.clear
                h = 40*pi/180;  w = 120*pi/180;                             %   rad - Path width/height
...
%                 loadComponent('poolScaleKiteAbney');                %   AR = 8; 8m span
                ...
                if q == 3
                    vhcl.setICsOnPath(0,PATHGEOMETRY,hiLvlCtrl.basisParams.Value,initGndStnPos,6.5*abs(flwSpd)*norm([1;0;0]))
                else
                ...
                initThrEl = atan2(z,sqrt(x^2+y^2));
                initThrAz = atan2(y,sqrt(x^2));
                
                las.setThrInitAng([-initThrEl initThrAz],'rad');
...
%                     vhcl.vStab.setCL(vhcl.vStab.CL.Value*.75,'')
%                     vhcl.vStab.setCD(vhcl.vStab.CD.Value*.75,'')
                ...
                if q == 2
                    tscSim = tsc1{i,j,k};
                    vel = tscSim.velCMvec.Data;
                    Ten = tscSim.gndNodeTenVecs.Data;
                    TenMag = sqrt(dot(Ten,Ten));
                    velMag = sqrt(dot(vel,vel));
                    figure
                    plotsq(tscSim.velCMvec.Time,velMag/0.77);
                    xlabel 'Time [s]'
                    ylabel 'Velocity Augmentation [$|v_{kite}|/|v_{tow}|$]'
                    yyaxis('right')
                    plot(tscSim.vhclAngleOfAttack)
                    ylim([0 15])
                    ylabel 'Vehicle Angle of Attack [deg]'
                    
                    figure
                    plotsq(tscSim.velCMvec.Time,TenMag);
                    xlabel 'Time [s]'
                    ...
                    plotsq(tscSim.eulerAngles.Time,tscSim.eulerAngles.Data(1,:,:)*180/pi+180);
                    plotsq(tscSim.eulerAngles.Time,tscSim.eulerAngles.Data(3,:,:)*180/pi-180);
                    xlabel('Time [s]')
                    ylabel('Angle [deg]')
                    legend ('Roll','Yaw')
                ...
                vhcl.animateSim(tscSim,0.2,'PathFunc',fltCtrl.fcnName.Value,'GifTimeStep',0.2,'SaveGif',1==1)%,'View',[0 0])
                
                if q == 2
%                     vhcl.animateSim(tsc1{i,j,k},0.2,'GifTimeStep',0.2,'SaveGif',1==1,'PathGeometry',1==1)%,'View',[0 0])
                    vhcl.animateSim(tscSim,.2,'TracerDuration',20,...
                        'GifTimeStep',.01,'PlotTracer',false,'FontSize',12,'Pause',1==0,...
                        'ZoomIn',1==0,'SaveGif',1==0,'endTime',20,'PlotLatLon',true);
                    figure('Position',[100 100 700 250]); hold on; grid on;
                    plot(tsc1{i,j,k}.rollSP-180,'-b','LineWidth',1.5)
                    plot(tsc1{i,j,k}.rollDeg-180,'--b','LineWidth',1.5)
                    plot(tsc1{i,j,k}.yawSP-180,'-r','LineWidth',1.5)
                    plot(tsc1{i,j,k}.yawDeg-180,'--r','LineWidth',1.5)
                    legend('SP','Response')
                    ylabel 'Attitude [deg]'
                    xlabel 'Time [s]'
                    ...
                    figure('Position',[100 100 700 250]); hold on; grid on;
                    plot(tsc1{i,j,k}.alSatDefl,'-b','LineWidth',1.5)
                    %             plot(tsc1{i}.ctrlSurfDefl.Time,tsc1{i}.ctrlSurfDefl.Data(:,2),'-g','LineWidth',1.5)
                    plot(tsc1{i,j,k}.elSatDefl,'-r','LineWidth',1.5)
                    plot(tsc1{i,j,k}.rudSatDefl,'-k','LineWidth',1.5)
...
% % yyaxis('left')
% %%
% figure('Position',[100 100 800 400]); hold on; grid on;
% 
% for i = 1:links
%     plot(tsc1{1,1,i}.phi*180/pi,'LineWidth',1.5,...
%         'DisplayName',sprintf('%d Tether Links',i))
% end
% xlabel 'Time [s]'
