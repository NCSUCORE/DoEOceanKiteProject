...
%             las.tetherLoadDisable;
            ...
            end_time = 40% tow_length/tow_speed;
            x_init = 4;
            y_init = 0;
            y_dot_init = 0;
            psi_init = 0;
...
%             lasElInit = las.initAng.Value(1)
%             lasAzInit = las.initAng.Value(2)
%             thrAttachInit = las.length.Value*[-cos(lasElInit)*cos(lasAzInit)...
%                  cos(lasElInit)*sin(lasAzInit) sin(lasElInit)]'+initGndStnPos;
%             thr.tether1.initGndNodePos.setValue(thrAttachInit,'m');
%             thr.tether1.initAirNodePos.setValue(vhcl.initPosVecGnd.Value(:)...
%                 +rotation_sequence(vhcl.initEulAng.Value)*vhcl.thrAttchPts_B.posVec.Value,'m');
            ...
            fltCtrl.startCtrl.setValue(40,'s')
...
fltCtrl.trimElevator.setValue(-4,'deg');
vhcl.rBridle_LE.setValue(vhcl.rBridle_LE.Value+[0.0127 0 0]','m')
            ...
            vhcl.animateSim(tsc1{i},0.01,'GifTimeStep',0.05,'SaveGif',1==1,'Pause',1==1,'View',[0 0])
            
            vhcl.animateSim(tsc1{1},0.2,'GifTimeStep',0.05,'SaveGif',1==1,'Pause',1==1,'View',[0 0])
