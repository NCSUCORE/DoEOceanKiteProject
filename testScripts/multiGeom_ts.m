...
% 2 - choose high level controller: 1 = const basis, 2 = variable alt, 3 = constant basis changing path geometry 
...
% 8 - plotting 
%%             1 2 3 4 5  6    7     8
simScenario = [1 1 1 1 1 false false true];
...
[a1,b1] = boothParamConversion(w,h);
a = w
b = h%   Path basis parameters
...
switch simScenario(2)                                   %   Flight Controller 
    case 1
        ...
        hiLvlCtrl.basisParams.setValue([a1,b1,el,0*pi/180,... %   Initialize basis parameters 
    ...
    case 3
        loadComponent('varGeom');                     %   Constant basis parameters
        el = asin(altitude/thrLength);                      %   rad - Initial elevation angle
...
if simScenario(3) == 1 || simScenario(3) == 2
    vhcl.setICsOnPath(.05,PATHGEOMETRY,[a1,b1,el,0*pi/180,thrLength],gndStn.posVec.Value,6.5*flwSpd)
else
...
if simScenario(7)
    ...
    elseif simScenario(3) == 2
        vhcl.animateSim(tsc,5,'TracerDuration',20,...
            'GifTimeStep',.01,'PlotTracer',true,'FontSize',12,'Pause',1==0,...
            'ZoomIn',1==0,'SaveGif',1==1,'GifFile','unwind2.gif',...
