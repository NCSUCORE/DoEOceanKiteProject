...
fltCtrl.elevatorConst.setValue(3,'deg');
...
fltCtrl.alphaCtrl.kp.setValue(.3,'(rad)/(kN)');     fltCtrl.Tmax.setValue(Tmax,'kN');
...
if runLin == 1
    ...
    cPV = logsout.getElement('closestPathVariable');
    lapNumS = logsout.getElement('lapNumS');
        x = zeros(length(snaps),max(lapNumS.Values.Data)-7);
    for j = 1:max(lapNumS.Values.Data)-7;

    [Idx1,Idx2] = getLapIdxs(tsc,j);
    ran = Idx1:Idx2-1;
    tic

    for i = 1 : length(snaps)
        x(i,j) = find(cPV.Values.Data(ran) >= snaps(i)-.005 & cPV.Values.Data(ran) <= snaps(i)+0.005, 1, 'last')+ran(1);
    end
    toc
...
%     toc
    ...
    tic
    linsys = linearize('OCTModel',op);
    toc
    %%
        snaps = 0.025:0.05:0.975;
...
%     wHzPert(i) = wHzPertMat(r(i),c(i),:)
    ...
    for i = 1:numEnt
        ...
        clear z
        if rem(i,length(snaps)) == 0
        scatter(real(p{:,i}.pole),imag(p{:,i}.pole),[],ones(length(p{:,i}.pole),1)*rem(snaps(end),0.5),'x')
        else
        scatter(real(p{:,i}.pole),imag(p{:,i}.pole),[],ones(length(p{:,i}.pole),1)*rem(snaps(rem(i,length(snaps))),0.5),'x')  
        end
...
x = Simulink.BlockDiagram.getInitialState('OCTModel')