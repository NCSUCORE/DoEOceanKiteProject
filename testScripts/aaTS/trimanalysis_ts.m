...
thrLength = 3.5%[10:5:50];                                  %  m - Initial tether length
flwSpd = .75%[0.25:.25:4];                                  %   m/s - Flow speed
el = 80*pi/180;                                             %   rad - Mean elevation angle
h = 10*pi/180;  w = 40*pi/180;                              %   rad - Path width/height
[a,b] = boothParamConversion(w,h);
desPitch = 10%[0:2:16]%;%8 % Desired Pitch in degrees
flowDir = 0%30*pi/180 % degrees
flowRot = [cos(flowDir) sin(flowDir) 0; -sin(flowDir) cos(flowDir) 0; 0 0 1];
if simScenario == 3.2
...
for ii = 1:numel(flwSpd)
    ...
    if simScenario >= 3
        ...
        fltCtrl.elevCmd.kp.setValue(5,'(deg)/(rad)');     fltCtrl.elevCmd.ki.setValue(1,'(deg)/(rad*s)');    %   Elevation angle inner-loop controller
...
%%  Set up critical system parameters and run simulation
    simParams = SIM.simParams;  simParams.setDuration(100,'s');  dynamicCalc = '';
