...
% 8 - plotting
%%             1 2 3 4 5  6    7     8
simScenario = [1 1 3 5 1 true true 1==0];
flwArray = 1e-9;[.1:.05:.5];
altArray = 50;[50:50:424];
thrArray = 100;[200:100:600];
...
craftSpeed = -1.5
yaw = 15
time = [0 500 515 530 600];
vel = craftSpeed*[ones(1,numel(time));...
    zeros(1,numel(time));...
    zeros(1,numel(time))]';
angVel = [zeros(1,numel(time));...
    zeros(1,numel(time));...
    0 0 yaw/15*pi/180 0 0]';
spiral = 1;
scale = 1;
q = 1
...
%                 fltCtrl.pitchCtrl.setValue(2,'');
        ...
        switch simScenario(5)                                   %   Environment
            case 1
                ...
                env.water.setflowVec(rotz(5)*[flwSpd 0 0]','m/s');           %   m/s - Flow speed vector
            case 2
...
%             save(strcat('output/powStudy/',filename),'tsc','vhcl','thr','fltCtrl','env','simParams','LIBRARY','gndStn')
        ...
        if simScenario(7)
            ...
            elseif simScenario(3) == 2
                ...
                vhcl.animateSim(tsc,10,'Pause',1==0,...
                    'GifTimeStep',.05,'PlotTracer',false,'FontSize',12,'ZoomInMove',1==1,...
...
plotsq(tsc.elevationAngle.Time,tsc.elevationAngle.Data,'LineWidth',1.5);
line([500 500],[-100 100],'Color','k','LineStyle',':','LineWidth',1.5)
legend('Response','Turn Initiated')
ylabel({'Elevation','Angle [deg]'})
ylim([38 42])
xlim([300 inf])
set(gca,'FontSize',15)
subplot(3,1,2); hold on; grid on;
plotsq(tsc.azimuthAngle.Time,tsc.azimuthAngle.Data,'LineWidth',1.5)
line([500 500],[-100 100],'Color','k','LineStyle',':','LineWidth',1.5)
...
plotsq(tsc.vhclSideSlipAngle.Time,tsc.vhclSideSlipAngle.Data,'LineWidth',1.5)
line([500 500],[-100 100],'Color','k','LineStyle',':','LineWidth',1.5)
