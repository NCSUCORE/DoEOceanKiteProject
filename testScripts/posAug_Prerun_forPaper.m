...
% 8 - Plotting
%%             1 2 3 4 5 6     7     8
simScenario = [1 1 1 3 1 1==0  0   1==0];
...
elevSet=5+asind(altSP/thrSweep);
elvSP=elevSet;
elvSet=elvSP;
elevStart=elvSP;
...
orientExitTime=slfExitTime+2000;
phaseShift=0;
...
%FOR POS SWEEP
altSweep =sind(elvSP).*thrSweep;%Kite Altitudes above seabed (m)
...
rollAmp=1; yawAmp=1; rollFreq=1; yawFreq=1; rollPhase=1; yawPhase=1;
yawAmp=yawAmp/0.7;
rollAmp=rollAmp/0.7;
...
%yawPhase=-0.05;
aRoll=[1 1];bRoll=1;aYaw=[1 1];bYaw=1;FREQ=0.01;
...
fltCtrlPath=fltCtrl;
% vhcl.portWing.gainCL.setValue(vhcl2.portWing.gainCL.Value,'1/deg');
% vhcl.stbdWing.gainCL.setValue(vhcl2.stbdWing.gainCL.Value,'1/deg');
% vhcl.portWing.gainCD.setValue(vhcl2.portWing.gainCD.Value,'1/deg');
% vhcl.stbdWing.gainCD.setValue(vhcl2.stbdWing.gainCD.Value,'1/deg');
% vhcl.hStab.gainCD.setValue(vhcl2.hStab.gainCD.Value,'1/deg');
% vhcl.hStab.gainCL.setValue(vhcl2.hStab.gainCL.Value,'1/deg');

%vhcl.mass.setValue(vhcl2.mass.Value,'kg')

%vhcl.rBridle_LE.setValue([0,0,0]','m')

%vhcl.turbMomentArms.setValue(vhcl.turbMomentArms.Value.*([1;1;0]),'m')
...
R.PATHPOWER(flwI,altI)=mean(tscP.turbPow.Data(tscP.lapNumS.Data==(max(tscP.lapNumS.Data)-1)));
R.PATHTENS(flwI,altI)=tPath;

            filename = sprintf(strcat('5degAOA_BASELINE_PATH_V-%.2f_Alt-%d_thr-%d.mat'),flwSpd,altitude,thrLength);
            if simScenario(6)
                save(strcat(simSavePath,filename),'tscP','vhcl','thr','fltCtrl','env','simParams','LIBRARY','gndStn','R')
            end
        end
    end
end
