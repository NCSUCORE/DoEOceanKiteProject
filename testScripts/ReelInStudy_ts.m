...
flwSpd = 0.15:0.05:0.15;                                     %   m/s - Flow speed
Vs = 0.01:0.02:0.25;
A = 2:-2:2;
...
for ii = 1:numel(flwSpd)
    for jj = 1:numel(Vs)
        for kk = 1:numel(A)
            ...
            if simScenario >= 4
                fltCtrl.LaRelevationSP.setValue(26,'deg');          fltCtrl.setNomSpoolSpeed(Vs(jj),'m/s');
                fprintf('\nFlow Speed = %.3f m/s;\tReel-in Speed = %.3f m/s\n',flwSpd(ii),-Vs(jj));
            else
            ...
            filename = sprintf(strcat('Turb%.1f_V-%.3f_EL-%.1f_Vs-%.2f_AoA-%d.mat'),simScenario,flwSpd(ii),mean(tsc.basisParams.Data(3,:,:))*180/pi,Vs(jj),A(kk));
%             filename = sprintf(strcat('LaR%.1f_V-%.3f_EL-%.1f_SP-%.1f_Wnch-%.2f.mat'),simScenario,flwSpd(ii),el*180/pi,fltCtrl.LaRelevationSP.Value,fltCtrl.nomSpoolSpeed.Value);
            fpath = 'D:\Results2\';
            save(strcat(fpath,filename),'tsc','vhcl','thr','fltCtrl','env','simParams','LIBRARY','gndStn')
        end
    end
end
% filename1 = sprintf('Reel-In_Study2.mat');
