...
% 8 - Plotting
%%             1 2 3 4 5 6     7     8
simScenario = [1 1 1 4 1 1==1  0   1==0];
...
yawlessTime=1000;
altSweep =sind(elevStart).*thrSweep;%Kite Altitudes above seabed (m)
transTime=0;
...
[aRoll,bRoll,aYaw,bYaw,FREQ] = getRollYawSetpointsFFT2(tscP,50);
   
    b2Roll=[0;bRoll];%Make size(a)=size(b) for indexing
    b2Yaw=[0;bYaw];%Make size(a)=size(b) for indexing

    thresh=0.0075;% If the power at the given harmonic is less than thresh, ignore that octave in the fourier series
...
%                 ten = max([max(airNode(ran)) max(gndNode(ran))]);
            ...
            filename = sprintf(strcat('PosAug_V-%.2f_AltSP-%d_thr-%d.mat'),flwSpd,altSP,thrLength);
                   end
    end
end
...
powPath=mean(tscP.elecPow.Data(tscP.lapNumS.Data==max(tscP.lapNumS.Data)-1))
tPath=mean(tscP.gndNodeTenVecs.mag.Data(tscP.lapNumS.Data==max(tscP.lapNumS.Data)-1))

%Calculate Orient Power
pOrientI2=find(min(abs(tsc.elecPow.Time-(orientExitTime)))==abs(tsc.elecPow.Time-(orientExitTime)));
pOrientI1=find(min(abs(tsc.elecPow.Time-(orientExitTime-period)))==abs(tsc.elecPow.Time-(orientExitTime-period)));

pOrient=mean(tsc.elecPow.Data(pOrientI1:pOrientI2))
tOrient=mean(tsc.gndNodeTenVecs.mag.Data(pOrientI1:pOrientI2))

%Calculate PosAugPower
pAugIndex=find(min(abs(tsc.elecPow.Time-(tsc.elecPow.Time(end)-period*3)))==abs(tsc.netPower.Time-(tsc.netPower.Time(end)-period*3)));

pPosAug=mean(tsc.elecPow.Data(pAugIndex:end))
