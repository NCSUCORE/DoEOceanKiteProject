...
% 8 - Plotting
%%             1 2 3 4 5 6     7     8
simScenario = [1 1 1 1 1 false false 1==1];
%%  Set Test Parameters
flwArray = .3;%0.1:0.05:0.5;                %   m/s - candidate flow speeds
...
for i = 1:numel(flwArray)
    for j = 1:numel(thrArray)
        ...
        altitude = thrLength*sind(30);         %   m - current altitude
        ...
        if ~isnan(el)
            ...
            if tsc.lapNumS.Data(end) >= 2
                ...
                R.Vavg(i,j) = mean(V(ran));
                R.AoA(i,j) = mean(squeeze(tsc.vhclAngleOfAttack.Data(:,:,ran)));
                airNode = squeeze(sqrt(sum(tsc.airTenVecs.Data.^2,1)))*1e-3;
                gndNode = squeeze(sqrt(sum(tsc.gndNodeTenVecs.Data.^2,1)))*1e-3;
                R.ten(i,j) = max([max(airNode(ran)) max(gndNode(ran))]);
                R.CL(i,j) = mean(CLtot(ran));   R.CD(i,j) = mean(CDtot(ran));
                R.Fdrag(i,j) = mean(Drag(ran)); R.Flift(i,j) = mean(Lift(ran));
                R.Ffuse(i,j) = mean(Fuse(ran)); R.Fthr(i,j) = mean(Thr(ran));   R.Fturb(i,j) = mean(Turb(ran));
                R.elevation(i,j) = el*180/pi;
                R.thrL(i,j) = thrLength; R.alt(i,j) = altitude; R.flw(i,j) = flwSpd;
                ...
                R.Pavg(i,j) = NaN;  R.AoA(i,j) = NaN;   R.ten(i,j) = NaN;
                R.CL(i,j) = NaN;    R.CD(i,j) = NaN;    R.Fdrag(i,j) = NaN;
                R.Flift(i,j) = NaN; R.Ffuse(i,j) = NaN; R.Fthr(i,j) = NaN;
                R.Fturb(i,j) = NaN; R.elevation(i,j) = el*180/pi;
                R.Pnet(i,j) = NaN;  R.Vavg(i,j) = NaN;
                R.thrL(i,j) = thrLength; R.alt(i,j) = altitude; R.flw(i,j) = flwSpd;
            end
        else
            R.Pavg(i,j) = NaN;  R.AoA(i,j) = NaN;   R.ten(i,j) = NaN;
