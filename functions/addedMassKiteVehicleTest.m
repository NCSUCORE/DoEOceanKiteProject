function [MA,v, MAfuse, vfuse] = addedMassKiteVehicleTest(cv,runname,savefigs)
...
%Do it Again with just the fuse
...
if(strcmp(cv.fuse.shape,'spheroid'))
    ...
    x = xo - a + sectWidth/2;
    r = b*sqrt(1-(x-xo)^2/a^2);
    vfuse.addSection(amsection(cv.fuse.secshape,r,r,sectWidth),[x;0;0],[0;0;0]);
    for i=2:1:cv.fuse.nsects % add circles in the +x direction towards the tail
        x = x + sectWidth;
        r = b*sqrt(1-(x-xo)^2/a^2);
        vfuse.addSection(amsection(cv.fuse.secshape,r,r,sectWidth),[x;0;0],[0;0;0]);
